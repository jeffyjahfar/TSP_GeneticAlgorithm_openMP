...
//  #pragma omp parallel for default( none ) shared( chromosome_file,trip )
...
void select( Trip trip[CHROMOSOMES], Trip parents[TOP_X] ) {
  ...
  #pragma omp parallel for default( none ) shared( parents,trip )
...
void populate( Trip trip[CHROMOSOMES], Trip offsprings[TOP_X] ) {
  ...
  #pragma omp parallel for default( none ) shared( offsprings,trip )
  for ( int i = 0; i < TOP_X; i++ )
